...
if(fs.existsSync(user_data)) { //check if file existsand parses new data to json file
...
app.post("/process_registration", function (request, response) {
    ...
    if (request.body.newpassword.length < 6) {// validate password
        errors.push("Password is too short!");
    }
    if (request.body.newpassword != request.body.newpassword_confirm) {  //confirm password 
        errors.push("Password confirmation does not match!");
    }
    if (errors.length == 0) { //validate if no errors
        let POST = request.body;
        username = POST['newuser']; //send data to userdata.json
          users_reg_data[username] = {};
          users_reg_data[username].name = POST['newfullname'];
          users_reg_data[username].password = POST['newpassword'];
          users_reg_data[username].email = POST['newemail'];
        reg_info_str = JSON.stringify(  users_reg_data); 
        fs.writeFileSync(user_data, reg_info_str, "utf-8");
...
app.post("/add_to_cart", function (request, response) { //post add to cart to session
    ...
    if (error == false) {
        ...
        var response = `${POST.quantity} Added`;
    }
    response = `${POST.quantity} Added`;
...
app.get("/complete_order", function (request, response) { //code from Phillip Sequira
